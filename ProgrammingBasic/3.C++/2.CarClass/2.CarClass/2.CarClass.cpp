...
//1.구매한 자동차의 색상을 지정하고 공장에서 주문한다.
//2.1에서 정한 색상으로 공장에서 자동차를 제조한다.
//3.3에서 생산된 자동차를 인수 받는다.
//4.자동차는 기어를 D에 넣어야 엑셀을 밟으면 속도가 증가한다.
//5.자동차는 엑셀을 밟으면 속도가 증가한다.
//6.자동차의 브레이크를 밟으면 속도가 감소한다.
//7.속도가 0이 되면 자동차는 멈춘다.
//8.자동차의 조작을 입력을 받아 선택하도록 한다.
//9.자동차의 기어를 P에 위치에 넣고 주행을 마친다.
